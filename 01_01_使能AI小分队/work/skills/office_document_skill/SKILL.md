...
description: 处理 LLM-WIKI 赛题中的 Word、PowerPoint、Excel 办公文档。Use when 需要对 .doc/.docx/.ppt/.pptx/.xls/.xlsx 文件执行 extract_text、extract_comments、filter_comments、count_comments、fix_comments、excel_analyze、provide_qa_context，支持结构化 todo 批注解析、Excel 表格分析，以及通过 soffice/libreoffice 转换老式二进制 Office 文件并在失败时安全降级。
---
...
- `excel_analyze`：返回基础 sheet/table 摘要到 `answer.datas` 和 `tables`。
