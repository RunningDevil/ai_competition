...
description: 处理 LLM-WIKI 赛题中的知识库问答。Use when 需要基于文件索引、Office 文本/批注/表格上下文、文本代码正文/TODO/风险上下文、主控通用文本兜底上下文执行 answer_from_context、answer_file_content_paths、answer_environment_info、answer_command_info、answer_excel_summary、answer_code_static_question、provide_answer_draft，并生成贴近比赛 JSON answer 格式的答案草稿。
---
...
- 根据正文内容返回相关文件名称和路径，并对内容型路径问题执行文件级证据聚合。
- 查询已允许访问范围内的环境信息。
...
- `context_blocks` 必须由 Office Agent、TextCode Agent 或主控编排Agent的通用文本兜底能力提供。
...
- `answer_file_content_paths`：根据正文内容和候选文件元数据返回相关 `docs/...` 路径；会把同一文件的多个正文/表格/批注命中聚合为文件级证据，并结合关键词扩展、同义词扩展和否定上下文过滤；如果题目明确询问数量，可返回 `{"count":n}`。
- `answer_environment_info`：从已允许访问的环境信息上下文中回答账号、地址、端口、密码等问题。
- `answer_command_info`：从常用命令或技术说明上下文中回答命令类问题；会优先识别命令目录/文件、命令特征行、fenced code block、多行续行命令、缩进行命令和嵌入式命令片段。
- `answer_excel_summary`：优先使用 `kind=table` 的上下文回答 Excel 摘要类问题。
- `answer_code_static_question`：优先使用 `kind=risk`、`kind=todo` 和文本代码正文回答静态代码问题；遇到“执行结果/返回值/输出结果”类题面时生成 `code_reasoning_tasks/*.json`，交由外层 CodeAgent 模型静态推演。
...
4. 使用 `retriever.py` 按文件名、路径、目录、后缀、关键词、上下文类型和问题意图对证据排序；内容型路径问题会进一步聚合同源块，生成 `kind=file_summary` 的文件级证据；命令类问题会对命令目录、命令后缀和 `ssh/gsql/psql/kubectl/docker/curl` 等命令信号加权。
5. 使用 `answer_builder.py` 生成比赛格式的 `answer`，并保留 `evidence` 和 `confidence` 供主控审计；命令类问题会优先返回与题面关键词匹配的完整命令块。
...
- 对真实代码执行结果类问题，不执行代码；只生成模型静态推演任务包，或在证据不足时返回结构化失败。
- 对其他后缀文件正文问答，依赖主控编排Agent传入通用文本兜底上下文；未提供上下文时返回结构化失败。
...
- 通用文本兜底读取由主控编排Agent提供，本 Skill 只消费 `context_blocks`，不主动读取文件。
- 当前没有真实代码执行沙箱；代码执行结果类问题通过 `code_reasoning_tasks/*.json` 交由外层 CodeAgent 静态推演。
- 当前 Excel 问答依赖 Office Agent 输出的文本块或表格摘要，不直接读取 Excel 文件。
- 内容型路径相关性判断是规则增强版，支持基础同义词和否定上下文，但不是向量检索或完整语义理解。
- 命令问答支持基础命令块抽取，但多段高度相似命令、题面与文档完全不同义、扫描件/图片中的命令仍可能需要后续增强。
- PDF 文本由主控编排Agent尽力读取后传入；可解码 PDF 可能命中，扫描版、图片版或复杂压缩 PDF 不保证。
