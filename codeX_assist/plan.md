...
| 知识问答Agent | 基于文件索引和办公文档Agent/文本代码Agent抽取结果检索相关内容；回答业务、技术、环境、常用命令、需求设计、Excel 摘要、代码静态问题等知识库问题；返回贴近比赛格式的结构化答案草稿。当前已落 `knowledge_qa_agent.md`、`knowledge_qa_skill/SKILL.md` 和 6 个核心脚本：统一 CLI、通用工具、上下文归一化、问题解析、证据排序、答案生成；后续继续增强通用文本兜底、复杂 Excel 问答和代码执行结果类问题。 | 进行中（基础脚本已落盘） | user1 |
...
| 输出与验证 | 保证答案 JSON 数组合法；保证答案顺序与输入问题一致；保证修复文件真实存在；准备公开样例和自造样例验证。当前已有答案格式校验器和最小 smoke test，尚未完成公开样例全量端到端、自造样例和 `result/output.md` 自验证记录。 | 进行中（基础校验已实现，全量验证未完成） | user1 |
...
| 模块 | 当前基础版风险 | 下一轮建议 |
| --- | --- | --- |
| 主控编排Agent | 题目分类仍是轻量关键词规则，对隐藏题中更绕的自然语言表达、跨文件组合问答、批注修复意图可能误分流；知识问答候选默认截断 Top 20，可能漏召回；候选文件批量安全检查只要召回中混入黑名单文件就可能整题拒绝，存在过拒风险；其他后缀文件目前不会进入正文抽取链路。 | 用公开样例+自造 20+ 题扩展分类规则；对黑名单候选改成安全过滤而非整题拒绝的策略评估；为其他后缀增加通用文本兜底；跑完整 group 端到端。 |
...
| 输出与验证 | 目前只做过模块级 smoke test 和主控最小 doc 数量测试，尚未用公开 `group-1` 全量端到端验证；修复文件真实存在性、答案顺序、日志 trace 完整性仍需完整跑。 | 下一轮优先跑公开样例全流程，记录失败题；补自造 Office/TextCode/安全/知识问答样例；修复后再准备 zip。 |
